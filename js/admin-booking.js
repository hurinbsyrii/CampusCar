document.addEventListener("DOMContentLoaded", function () {
    ...
    const statusTransitions = {
        'Pending': [
            { value: 'Confirmed', label: 'Confirmed', description: 'Driver has accepted booking', icon: 'fa-check' },
            { value: 'Cancelled', label: 'Cancelled', description: 'Cancel this booking', icon: 'fa-times' }
        ],
        'Confirmed': [
            { value: 'Paid', label: 'Paid', description: 'Payment has been received', icon: 'fa-money-bill-wave' },
            { value: 'Cancelled', label: 'Cancelled', description: 'Cancel confirmed booking', icon: 'fa-times' }
        ],
        'Paid': [
            { value: 'Completed', label: 'Completed', description: 'Ride has been completed', icon: 'fa-flag-checkered' },
            { value: 'Cancelled', label: 'Cancelled', description: 'Cancel paid booking', icon: 'fa-times' }
        ],
        'Completed': [],
        'Cancelled': []
    };
    ...
    document.addEventListener("click", function (e) {
        ...
        if (e.target.closest(".view-btn")) {
            ...
            const bookingInfo = {
                ...
                route: row.querySelectorAll('.route-info p')[0].textContent + ' → ' + 
        ...
        if (e.target.closest(".update-btn")) {
            ...
            console.log("Update status clicked for booking:", bookingId, "Current status:", currentStatus);
        ...
        if (e.target.closest(".cancel-btn")) {
            ...
            const bookingId = btn.dataset.id;
            const currentStatus = btn.dataset.currentStatus;
            console.log("Cancel clicked for booking:", bookingId, "Current status:", currentStatus);
    ...
    function showStatusModal(bookingId, currentStatus, isCancelAction = false) {
        ...
        if (isCancelAction) {
            ...
            if (filteredTransitions.length === 0) {
                statusHelp.textContent = "This booking cannot be cancelled from its current status.";
                statusHelp.style.color = "#dc3545";
                statusHelp.style.display = 'block';
                return;
            }
    ...
    if (window.innerWidth <= 768) {
        ...
        const headers = ['Booking & User', 'Ride Details', 'Payment Info', 'Date & Time', 'Status', 'Actions'];
